    ...
    <div class="container container-margin mb-5">
        <div class="row justify-content-center">
            <div class="col-xl-6 col-lg-6 col-md-9 bg-body-tertiary py-3">
                <h5 class="display-6 fw-bold text-center">Edit User</h5>
                <form action="update_user.php" method="post" enctype="multipart/form-data">
                    ...
                    ?>
                        ...
                        <select name="kelasid" id="kelasid" class="form-control mb-2">
                            ...
                            // Mengambil data kelas dengan INNER JOIN antara tabel kelas dan user
                            $sql_kelas = mysqli_query($conn, "SELECT kelas.* FROM kelas INNER JOIN user ON kelas.kelasid = user.kelasid WHERE user.role = '$role'");

                            // Memeriksa apakah kueri pertama mengembalikan hasil
                            if (mysqli_num_rows($sql_kelas) > 0) {
                                while ($data_kelas = mysqli_fetch_array($sql_kelas)) {
                            ?>
                                    <option value="<?= $data_kelas['kelasid'] ?>" <?php if ($data_kelas['kelasid'] == $data['kelasid']) {
                                                                                        echo 'selected';
                                                                                    } ?>>
                                ...
                                // Jika tidak ada data yang ditemukan, tampilkan opsi "Tidak Ada"
                                ?>
                                <option value="" selected>Tidak Ada</option>
                            <?php
                            }

                            // Mengambil data kelas dari tabel kelas tanpa INNER JOIN
                            $sql_kelas_2 = mysqli_query($conn, "SELECT * FROM kelas");
                            while ($data_kelas_2 = mysqli_fetch_array($sql_kelas_2)) {
                            ?>
                                <option value="<?= $data_kelas_2['kelasid'] ?>" <?php if ($data_kelas_2['kelasid'] == $data['kelasid']) {
                                                                                    echo 'selected';
                                                                                } ?>><?= $data_kelas_2['namakelas'] ?></option>
                            <?php
                            }
                            ?>
                        ...
                        <select name="jurusanid" id="jurusanid" class="form-control mb-2">
                            ...
                            // Mengambil data jurusan dengan INNER JOIN antara tabel jurusan dan user
                            $sql_jurusan = mysqli_query($conn, "SELECT jurusan.* FROM jurusan INNER JOIN user ON jurusan.jurusanid = user.jurusanid WHERE user.role = '$role'");

                            // Memeriksa apakah kueri pertama mengembalikan hasil
                            if (mysqli_num_rows($sql_jurusan) > 0) {
                                while ($data_jurusan = mysqli_fetch_array($sql_jurusan)) {
                            ?>
                                    <option value="<?= $data_jurusan['jurusanid'] ?>" <?php if ($data_jurusan['jurusanid'] == $data['jurusanid']) {
                                                                                            echo 'selected';
                                                                                        } ?>>
                                        <?= $data_jurusan['kepanjangan'] ?>
                                    </option>
                                <?php
                                }
                            } else {
                            ...
                            // Mengambil data jurusan dari tabel jurusan tanpa INNER JOIN
                            $sql_jurusan_2 = mysqli_query($conn, "SELECT * FROM jurusan");
                            while ($data_jurusan_2 = mysqli_fetch_array($sql_jurusan_2)) {
                            ?>
                                <option value="<?= $data_jurusan_2['jurusanid'] ?>" <?php if ($data_jurusan_2['jurusanid'] == $data['jurusanid']) {
                                                                                        echo 'selected';
                                                                                    } ?>><?= $data_jurusan_2['kepanjangan'] ?></option>
                            <?php
                            }
                            ?>
                            <option value="">Tidak Ada</option>
